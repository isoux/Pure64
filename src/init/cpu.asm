...
x2apic_read:
	push rdx
	rdmsr				; Read from the x2APIC, result in EDX:EAX
...
x2apic_write:
	...
	push rax
	mov rdx, rax
	shr rdx, 32			; Shift upper 32 bits to lower 32 bits
	wrmsr				; Write EDX:EAX to the x2APIC
	pop rax
	pop rdx
	ret
; -----------------------------------------------------------------------------
...
x2APIC_ID		equ 0x802	; ID Register
x2APIC_VER		equ 0x803	; Version Register
; 0x004 - 0x007 are Reserved
x2APIC_TPR		equ 0x808	; Task Priority Register
x2APIC_APR		equ 0x809	; Arbitration Priority Register
x2APIC_PPR		equ 0x80A	; Processor Priority Register
x2APIC_EOI		equ 0x80B	; End Of Interrupt
x2APIC_RRD		equ 0x80C	; Remote Read Register
x2APIC_LDR		equ 0x80D	; Logical Destination Register
x2APIC_DFR		equ 0x80E	; Destination Format Register
x2APIC_SPURIOUS		equ 0x80F	; Spurious Interrupt Vector Register
x2APIC_ISR		equ 0x810	; In-Service Register (Starting Address)
x2APIC_TMR		equ 0x818	; Trigger Mode Register (Starting Address)
x2APIC_IRR		equ 0x820	; Interrupt Request Register (Starting Address)
...
x2APIC_TMRCURRCNT	equ 0x839	; Current Count Register (for Timer)
; 0x03A - 0x03D are Reserved
x2APIC_TMRDIV		equ 0x83E	; Divide Configuration Register (for Timer)
x2APIC_SELFIPI		equ 0x83F	; SELF IPI
; 0x83F is Reserved
